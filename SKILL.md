...
description: Master Taobao with seller vetting, price negotiation, deal hunting strategies, and browser-based product/cart/coupon checks for China's largest C2C marketplace.
metadata:
  clawdbot:
    emoji: "🛍️"
    requires:
...
When the task requires live page interaction, use the shared **browser-commerce-base** workflow:
- public search/detail pages → isolated `openclaw` browser
- cart/coupon/order/member pages → `user` profile only when login state matters
- always `snapshot` before acting, and re-snapshot after SKU/filter/page changes
- capture evidence screenshots when comparing products or recommending a seller

For browser extraction, prioritize:
- 标题
- 当前价 / 到手价
- 店铺类型
- 销量 / 评价提示
